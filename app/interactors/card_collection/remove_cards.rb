class CardCollection::RemoveCards
  ...
  def call
    ...
    card_ids.each do |card_id|
      ...
      if (card_lists = CardList.where(card_id: card_ids, card_listable_type: 'CardCollection', card_listable_id: card_collection.id)).any?
        card_lists.each do |card_list|
          ...
          card_list_ids << [card_list.id] if card_list.foils_number.to_i + card_list.number.to_i == 0
        end
      end
    end
