...
class Alternative < ApplicationRecord
  ...
  def set_type_to_card
    name_mapping = { 'Fire / Ice' => 'double_card', 'Nezumi Graverobber / Nighteyes the Desecrator' => 'flip_card' }
    return card.update(alternative_type: name_mapping[card.name]) if name_mapping[card.name].present?
    card.update(alternative_type: {
